...
void legsGoBackToStartPosition() {
	float gearRatio = 11/6;
	int currentPosition = nMotorEncoder[motorB];
	//nxtDisplayTextLine(0, "%d", nMotorEncoder[motorB]);
	while (currentPosition > gearRatio * 180) currentPosition -= gearRatio * 360;
	while (currentPosition < gearRatio * -180) currentPosition += gearRatio * 360;
	nxtDisplayTextLine(0, "%d", currentPosition);
	int desiredPosition = nMotorEncoder[motorB] - currentPosition;
	while (nMotorEncoder[motorB] > desiredPosition) motor[motorB] = -50;
	while (nMotorEncoder[motorB] < desiredPosition) motor[motorB] = 50;
	motor[motorB] = 0;

	currentPosition = nMotorEncoder[motorC];
	//nxtDisplayTextLine(0, "%d", nMotorEncoder[motorC]);
	while (currentPosition > gearRatio * 180) currentPosition -= gearRatio * 360;
	while (currentPosition < gearRatio * -180) currentPosition += gearRatio * 360;
	nxtDisplayTextLine(0, "%d", currentPosition);
	desiredPosition = nMotorEncoder[motorC] - currentPosition;
	while (nMotorEncoder[motorC] > desiredPosition) motor[motorC] = -50;
	while (nMotorEncoder[motorC] < desiredPosition) motor[motorC] = 50;
	motor[motorC] = 0;
...
task main()
{
	...
	while (true) {
		...
		if (joystick.joy1_Buttons == R1) turn_right();
			...
			desiredPosition = (desiredPosition == 0) ? 500 : 0;
