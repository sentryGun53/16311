...
task trajectory_following()
{
	while(1)
	{
		...
		float markerX = robot_X + MARKERDISTANCE * cos(robot_TH);
		float markerY = robot_Y + MARKERDISTANCE * sin(robot_TH);
...
task main()
{
  /* Reset encoders and turn on PID control */
	...
	// Need to now decide where to head
	//startTask(trajectory_following);

	/* // This part no longer relevant
	for(int i = 0; i < 3; i++)
	{
